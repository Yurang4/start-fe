...
const total_page_num = document.querySelector('.total-page');

var page = 1;
var image_per_page = 3;
var total_page = Math.ceil(todayPhoto.length / image_per_page);

var start_image_index = (page - 1) * image_per_page;
var end_image_index = (start_image_index + image_per_page) - 1;
var max_index = todayPhoto.length - (image_per_page - (todayPhoto.length % image_per_page));

page_num.innerHTML = page;
total_page_num.innerHTML = total_page;
...
function prev_page() {
  if(page > 1){
    ...
    page_num.innerHTML = page;
    str = '';
    start_image_index = (page - 1) * image_per_page;
    end_image_index = (start_image_index + image_per_page) - 1;
  }
  print_image();
}
...
function next_page() {
  if(page < total_page){
    ...
    if(end_image_index > max_index){
      end_image_index = max_index;
    }
  }
  print_image();
}
...
function print_image() {
  for(var i=start_image_index; i<=end_image_index; i++){
...
button_prev.addEventListener('click', prev_page);
button_next.addEventListener('click', next_page);
print_image();
